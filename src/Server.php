		...
		try{
			...
			if(VersionInfo::IS_DEVELOPMENT_BUILD){
				if(!$this->configGroup->getPropertyBool("settings.enable-dev-builds", false)){
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::pocketmine_server_devBuild_error1(VersionInfo::DISTRO_NAME)));
					...
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::pocketmine_server_devBuild_error5("https://github.com/UnnamedNetwork/UNWDS/releases")));
					$this->forceShutdown();
				...
				$this->logger->warning($this->language->translate(KnownTranslationFactory::pocketmine_server_devBuild_warning1(VersionInfo::DISTRO_NAME)));
			...
			$this->logger->info($this->getLanguage()->translate(KnownTranslationFactory::pocketmine_server_info(
				$this->getDistroName(),
				(VersionInfo::IS_DEVELOPMENT_BUILD ? TextFormat::YELLOW : "") . $this->getDistroVersion() . TextFormat::RESET
			)));
			$this->logger->info($this->getLanguage()->translate(KnownTranslationFactory::pocketmine_server_license($this->getDistroName())));
			...
			$this->logger->info($this->getLanguage()->translate(KnownTranslationFactory::pocketmine_server_donate(TextFormat::AQUA . "https://github.com/UnnamedNetwork/UNWDS" . TextFormat::RESET)));
			...
			if($this->configGroup->getPropertyBool("auto-report.enabled", true)){
				...
				if($report){
					...
					$reply = Internet::postURL($url, [
						"report" => "no", // disable this
						"name" => $this->getDistroName() . " " . $this->getDistroVersion(),
						"email" => "blackhole", // currently, we don't have any email address for crash report, you can use the issues tracker in Github instead.
