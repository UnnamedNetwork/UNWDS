	...
	public function __construct(\ClassLoader $autoloader, \AttachableThreadedLogger $logger, string $dataPath, string $pluginPath){
		...
		try{
			...
			if(\pocketmine\IS_DEVELOPMENT_BUILD and !((bool) $this->getProperty("settings.enable-dev-builds", false))){
				...
				$this->logger->emergency($this->baseLang->translateString("pocketmine.server.devBuild.error5", ["https://github.com/UnnamedNetwork/UNWDS/releases"]));
				$this->forceShutdown();
				return;
			}
			...
			$this->logger->warning($this->getLanguage()->translateString("pocketmine.compatibilitymode.info"));
